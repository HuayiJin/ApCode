...
In the $n$ chunks of each stripe, we use $m$ parity chunks, and there are $s \times t$ additional symbols in the remaining $n-m$ chunks for encoding. Our goal is to provide different reliability for important and general data, so we assume that normal data can tolerate damage of up to $m$ chunks, while important data can tolerate damage of up to $m+s$ chunks. Figure \ref{TEST-apcode-75222} shows an example of an approximate code for $n = 7$, $r = 4$, $m = 2$, $t=2$ and $s = 2$.
\begin{figure}[ht]
\centering
\includegraphics[width=0.4\textwidth]{photo/TEST-apcode-75222.JPG}
\caption{TEST--A sample of Approximate Code with n = 7, r = 5, m = 2, t=2 and s = 2.}
\label{TEST-apcode-75222}
\end{figure}

\subsection{Design of Approximate Code}
We label $n$ disks from $D_0$ to $D_{n-1}$. Among them, $D_{n-m}$ to $D_{n-1}$ are parity disks. We label the data disks with $d_{i,j}$, the important data parity sectors with $x_{i,j}$ and the normal parity check sectors with $y_{i,j}$. The following equation represents the method to calculate $x_{i,j}$, $y_{i,j}(i<t)$ and $y_{i,j}(i \geqslant t)$

\begin{equation}
    x_{i,j} = \sum_{k=0}^{n-m-s-1} \alpha_k^{j+1} d_{i,k}
\end{equation}
\begin{equation}
    \mathop{y_{i,j}}\limits_{i<t} = \sum_{k=0}^{n-m-s-1} \alpha_k^{s+j+1} d_{i,k}
\end{equation}
